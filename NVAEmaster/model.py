...
class Cell(nn.Module):
    def __init__(self, Cin, Cout, cell_type, arch, use_se, dropout_p=0.0):
    ...
    def forward(self, s):
        ...
        if self.dropout_p > 0.0:
            s = F.dropout2d(s, p=self.dropout_p, training=self.training or self.force_dropout)
        return skip + 0.1 * s
...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance):
        ...
        self.mc_dropout_p = getattr(args, "mc_dropout_p", 0.0)
        self.mc_dropout_dec_p = getattr(args, "mc_dropout_dec_p", self.mc_dropout_p)
        ...
        for b in range(self.num_preprocess_blocks):
            for c in range(self.num_preprocess_cells):
                if c == self.num_preprocess_cells - 1:
                    ...
                    cell = Cell(num_ci, num_co, cell_type='down_pre', arch=arch, use_se=self.use_se,dropout_p=self.mc_dropout_p)
                    mult = CHANNEL_MULT * mult
                else:
                    arch = self.arch_instance['normal_pre']
                    num_c = self.num_channels_enc * mult
                    cell = Cell(num_c, num_c, cell_type='normal_pre', arch=arch, use_se=self.use_se,dropout_p=self.mc_dropout_p)
        ...
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[s]):
                for c in range(self.num_cell_per_cond_enc):
                    ...
                    cell = Cell(num_c, num_c, cell_type='normal_enc', arch=arch, use_se=self.use_se,dropout_p=self.mc_dropout_p)
            ...
            if s < self.num_latent_scales - 1:
                ...
                num_ci = int(self.num_channels_enc * mult)
                num_co = int(CHANNEL_MULT * num_ci)
                cell = Cell(num_ci, num_co, cell_type='down_enc', arch=arch, use_se=self.use_se, dropout_p=self.mc_dropout_p)
                enc_tower.append(cell)
                mult = CHANNEL_MULT * mult
    ...
    def init_decoder_tower(self, mult):
        ...
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
                ...
                if not (s == 0 and g == 0):
                    for c in range(self.num_cell_per_cond_dec):
                        ...
                        cell = Cell(num_c, num_c, cell_type='normal_dec', arch=arch, use_se=self.use_se, dropout_p=self.mc_dropout_dec_p)
            ...
            if s < self.num_latent_scales - 1:
                ...
                cell = Cell(num_ci, num_co, cell_type='up_dec', arch=arch, use_se=self.use_se, dropout_p=self.mc_dropout_dec_p)
                dec_tower.append(cell)
                mult = mult / CHANNEL_MULT
        ...
        for b in range(self.num_postprocess_blocks):
            for c in range(self.num_postprocess_cells):
                if c == 0:
                    ...
                    num_ci = int(self.num_channels_dec * mult)
                    num_co = int(num_ci / CHANNEL_MULT)
                    cell = Cell(num_ci, num_co, cell_type='up_post', arch=arch, use_se=self.use_se, dropout_p=self.mc_dropout_dec_p)
                    mult = mult / CHANNEL_MULT
                else:
                    arch = self.arch_instance['normal_post']
                    num_c = int(self.num_channels_dec * mult)
                    cell = Cell(num_c, num_c, cell_type='normal_post', arch=arch, use_se=self.use_se, dropout_p=self.mc_dropout_dec_p)
    ...
    def forward(self, x, return_latents=False, return_posterior_means=False):
        s = self.stem(2 * x - 1.0)
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    z, _ = dist.sample()
                    sampled_z.append(z)
                    posterior_means.append(dist.mu)
                    log_q_conv = dist.log_p(z)
                    # apply NF
                    for n in range(self.num_flows):
        ...
        if return_latents and return_posterior_means:
            return logits, log_q, log_p, kl_all, kl_diag, sampled_z, posterior_means
        if return_latents:
            return logits, log_q, log_p, kl_all, kl_diag, sampled_z
